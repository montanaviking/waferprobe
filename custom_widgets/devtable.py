...
class DevTable(QtWidgets.QTableWidget):
	...
	def __init__(self,parent=None,):
		...
		self.hheaderview=QtWidgets.QHeaderView(QtCore.Qt.Horizontal)
		self.hheaderview.setSectionsClickable(True)
	...
	# parameter_standins is a dictionary of the form parameter_standins[parameter to substitute]=parameter name in dataset -> i.e. this allows one to specify a column (parameter) label of one name but use a different parameter name in the dataset.
	# The index is the parameter label to appear in the column label (hheader) and the value is the actual data parameter type
	# parameter_standins is the type of transfer curve e.g.: parameter_standins["|Idmax|"]="|Idmax| single transfer", we want headers of |Idmax| to stand in for things such as "|Idmax| single transfer" or |Idmax| dual 1st, etc....
	# parameter_standins is a dictionary of form transfertypcurve[A]=B where A is a measured or calculated data type - parameter and B is the generic parameter which is to stand in for A e.g. parameter_standins["ORTHRatio |Idmax| single"]= "ORTHRatio |Idmax|" would
	# substitute "ORTHRatio |Idmax|" for the hheader entry of "ORTHRatio |Idmax| single" and get data from the parameter type "ORTHRatio |Idmax| single"

	def setup(self,forcecleartable=False,hheaders=None,vheaders=None,data=None,parameter_standins=None,useoldparameter_standins=False):
		...
		if parameter_standins==None and useoldparameter_standins: parameter_standins=self.parameter_standins
		self.parameter_standins=parameter_standins

		#if parameter_standins!=None and len(set([x[1] for x in list(parameter_standins.items())]))!=len(parameter_standins): raise ValueError("ERROR! parameter values in parameter_standins must be unique")

		if hheaders==None or data==None or vheaders==None or forcecleartable:
			self.clear()
			self.setRowCount(0)
			self.setColumnCount(0)
			return None
		if parameter_standins!=None:										# set up headers if the transfer curve type has been chosen
			# replace data-derived headers (parameters list) with that modified by parameter_standins
			hp = [parameter_standins[h] for h in parameter_standins if h in hheaders]
			hheaders=[h for h in hheaders if h not in parameter_standins]
			hheaders.extend(hp)
		...
		for ir in range(0,self.rowCount()):
			addeditem=sItem()
			addeditem.setText(vheaders[ir])                     # device name
			self.setItem(ir,0,addeditem)						# the first column of data is the device name
			for ic in range(1,self.columnCount()):
				addeditem=sItem()
				if parameter_standins!=None:						# allow the use of "generic" parameter names and stand-ins for them
					#dataparameter=[parameter_standins[t] for t in parameter_standins if hheaders[ic] == t][0]				# dataparameter is the actual data parameter type - normally specified by the column label i.e. the horizontal header but in this case we specified a "stand in"
					dp=[t for t in parameter_standins if hheaders[ic] == parameter_standins[t]]
					if len(dp)>0:
						dataparameter=dp[0]				# dataparameter is the actual data parameter type - normally specified by the column label i.e. the horizontal header but in this case we specified a "stand in"
						if dataparameter in data[vheaders[ir]] and is_number(formatnum(data[vheaders[ir]][dataparameter],precision=2)):
							if dataparameter=='X' or dataparameter=='Y' or dataparameter=='total gate width (um)':      # these parameters are to be formatted as integers
								addeditem.setText(formatnum(data[vheaders[ir]][dataparameter],type='int'))                # format as integer for this item's entry into the table
							else:
								addeditem.setText(formatnum(data[vheaders[ir]][dataparameter],precision=2))             # format as float
						else: addeditem.setText('no data')
					elif hheaders[ic] in data[vheaders[ir]] and is_number(formatnum(data[vheaders[ir]][hheaders[ic]],precision=2)):
						addeditem.setText(formatnum(data[vheaders[ir]][hheaders[ic]],precision=2))
						if hheaders[ic]=='X' or hheaders[ic]=='Y' or hheaders[ic]=='total gate width (um)':
							addeditem.setText(formatnum(data[vheaders[ir]][hheaders[ic]],type='int'))
					else: addeditem.setText('no data')
				else:	# use the horizontal header (column label) as the data parameter type
					if hheaders[ic] in data[vheaders[ir]] and is_number(formatnum(data[vheaders[ir]][hheaders[ic]],precision=2)):
						if hheaders[ic]=='X' or hheaders[ic]=='Y' or hheaders[ic]=='total gate width (um)':     # these parameters are to be formatted as integers
							addeditem.setText(formatnum(data[vheaders[ir]][hheaders[ic]],type='int'))           # format as integer for this item's entry into the table
						else: addeditem.setText(formatnum(data[vheaders[ir]][hheaders[ic]],precision=2))        # format as float
					else: addeditem.setText('no data')
	...
	def sortitems(self, icol=None):
		...
		if (mods==QtCore.Qt.ShiftModifier or mods==QtCore.Qt.ControlModifier) and icol>0: 			# want to reserve shift mouseclick for to copy to select columns for clipboard
			...
			if not duplicatecolumn:
				...
				self.setCursor((QtCore.Qt.ArrowCursor))		# signal user that selection is complete
...
# copy the selected data to the clipboard
	def keyPressEvent(self,ke):
		...
		#### find selected devices using ctrl-f
			...
			findpop.show()
		###################
		# elif ke.key()==QtCore.Qt.Key_V and mods==QtCore.Qt.ControlModifier:			# cntl-v to paste column (when functionality is available
		# 	clipbtopaste=QtWidgets.QApplication.clipboard()
		# 	if clipbtopaste.text()!="":
		# 		self.ctext=clipbtopaste.text()                              # need to pass this way to avoid deletion of clipboard
		# 		self.PasteColumn.emit(self.ctext)                                # emit signal to captured by __pastecolumn() method in actions_comparator.py
		# 	else:
...
# this method is called whenever the user clicks a cell
	def vheadertext(self,irow,icol):
		...
		if mods==QtCore.Qt.ShiftModifier:
			...
			clipb=QtWidgets.QApplication.clipboard()					# send cell contents to clipboard
...
# the parent class is DevTable in this file
# the parent().parent() is DeviceListing() in actions_devlisting.py
class Boolean_find(Ui_BooleanFinder,QtWidgets.QDialog):
	...
	def newsearch(self):
		if self.boolean_device_finder.text()=="": vheaders=self.alldevices_vheaders
			...
			#print("from line 323 in devtable.py boolean", self.boolean_device_finder.text())
			vheaders=[dev for dev in self.alldevices_vheaders if parse_rpn(expression=self.boolean_device_finder.text(),targetfilename=dev,parentwidget=self.boolean_device_finder)]
		#print("from line 323 in devtable.py standins= ", self.standins)
		self.parent().setup(vheaders=vheaders,useoldparameter_standins=True)
	...
	# def keyPressEvent(self,ke):
	# 	print("from line 330 in devtable.py boolean find keypress")
	# 	mods=QtWidgets.QApplication.keyboardModifiers()
	# 	if mods==QtCore.Qt.ControlModifier:                           # user hit cntr-C
	# 		print("from line 333 in devtable.py boolean find cntl-c")
	# 		clipb=QtWidgets.QApplication.clipboard()							                        # set up clipboard
	# 		clipb.setText(self.boolean_device_finder.text())                                        # move text to clipboard
	# 		print("clipb",clipb.text())
	# 	ke.ignore()

	def closeEvent(self,e):
		#print("from line 326 in devtable.py standins= ",self.standins)
		...
		self.parent().setup(vheaders=self.alldevices_vheaders,useoldparameter_standins=True)  # restore all former devices after find to listing
		e.accept()