...
##############################################################################################
## pulsed drain time domain tests
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		if devicegood==True:	# measure only good devices
			iv.measure_hysteresistimedomain_pulseddrain_dual_backgated(Vgs=-10., timestep=0.01, Vdsquiescent=0., timequiescent=0.5, timeend=40., Vds_start=-1., Vds_stop=-1., Vds_step=0, gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_pulsedtimedomain4200_pulseddrain_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),timepointsperdecade=30,xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='Tq_0.5Vds-1VdsQ0Vgs0')
	pr.move_nextsite()
pr.move_separate()
# #del cascade
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=25., quiescenttime=0., startstopzero=True, Vds=-0.01,Vgs_start=-5,Vgs_stop=5,Vgs_step=.2,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew25Vgs5Vds-0.01')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=10., quiescenttime=0., startstopzero=True, Vds=-0.01,Vgs_start=-5,Vgs_stop=5,Vgs_step=.2,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew10Vgs5Vds-0.01')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=1, quiescenttime=0,startstopzero=True, Vds=-0.01,Vgs_start=-5,Vgs_stop=5,Vgs_step=0.2,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew1Vgs5Vds-0.01')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=25., quiescenttime=0, startstopzero=True, Vds=-0.01,Vgs_start=-10,Vgs_stop=10,Vgs_step=.5,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew25Vgs10Vds-0.01')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=10., quiescenttime=0, startstopzero=True, Vds=-0.01,Vgs_start=-10,Vgs_stop=10,Vgs_step=.5,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew10Vgs10Vds-0.01')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=1., quiescenttime=0,startstopzero=True, Vds=-0.01,Vgs_start=-10,Vgs_stop=10,Vgs_step=.5,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew1Vgs10Vds-0.01')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=25., quiescenttime=60, startstopzero=True, Vds=-1.5,Vgs_start=-5,Vgs_stop=5,Vgs_step=.2,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew25Vgs5Vds-1.5')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=10., quiescenttime=0, startstopzero=True, Vds=-1.5,Vgs_start=-5,Vgs_stop=5,Vgs_step=.2,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew10Vgs5Vds-1.5')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=1, quiescenttime=0,startstopzero=True, Vds=-1.5,Vgs_start=-5,Vgs_stop=5,Vgs_step=0.2,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew1Vgs5Vds-1.5')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=25., quiescenttime=0, startstopzero=True, Vds=-1.5,Vgs_start=-10,Vgs_stop=10,Vgs_step=.5,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew25Vgs10Vds-1.5')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivtransferloop_4sweep_controlledslew_dual_backgated(Vgsslewrate=10., quiescenttime=0, startstopzero=True, Vds=-1.5,Vgs_start=-10,Vgs_stop=10,Vgs_step=.5,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew10Vgs10Vds-1.5')
	pr.move_nextsite()
...
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		if devicegood==True:	# measure only good devices
			...
			iv.writefile_ivtransferloop_4sweep_dual(pathname=pathname,devicenames=pr.devicenamesatlevel(),wafername=pr.wafername(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier='slew1Vgs10Vds-1.5')
	pr.move_nextsite()


## drain voltage hysteresis loops now probe devices########################################################################################
pr = CascadeProbeStation(rm,pathname,"Hf4meas3_plan","correction off")                                                               # setup Cascade
pr.move_plan_index()					# move to first site
firsttime=True
totalnumbercleans=0
while pr.get_probingstatus()=="probing wafer":
	...
	if "proberesistancetest" in pr.devicenamesatlevel()[0]:         # then this is a probe resistance test device
		...
		devicegood=True
		if devicegood==True:	# measure only good devices
			iv.measure_ivfoc_Vdsloop_controlledslew_dual_backgated(Vdsslewrate=1, quiescenttime=0,startstopzero=True, Vds_start=-1, Vds_stop=1, Vds_npts=21, Vgs_start=-10, Vgs_stop=-10, Vgs_npts=1,gatecomp=gatecomp,draincomp=draincomp)
			iv.writefile_ivfoc_Vdsloop_dual(pathname=pathname,wafername=pr.wafername(),devicenames=pr.devicenamesatlevel(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier="Vgs0Vds1+-+-")
	pr.move_nextsite()
...
# 			iv.writefile_ivfoc_Vdsloop_dual(pathname=pathname,wafername=pr.wafername(),devicenames=pr.devicenamesatlevel(),xloc_probe=pr.x(),yloc_probe=pr.y(),devicenamemodifier="Vgs0Vds2+-+-")
# 	pr.move_nextsite()
#######################################################################################################
