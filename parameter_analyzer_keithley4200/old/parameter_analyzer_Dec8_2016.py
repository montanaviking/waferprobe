...
# set up of parameter analyzer
	def __init__(self,rm):                                  # setup of Keithley parameter analyzer
		try:
			self.__parameteranalyzer = rm.open_resource('GPIB0::2::INSTR')
		...
		self.__parameteranalyzer.query_delay=.2            # set up query delay to prevent errors
...
# set constant bias and read drain and gate currents. This is useful for taking S-parameter measurements
	def fetbiason_topgate(self, Vgs=None, Vds=None, gatecomp=1.E-5, draincomp=0.1,maxchangeId=0.05,maxtime=10.,timeiter=0.2):                 # set constant bias and measure gate and drain currents. Bias is left on at exit
	# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("IT1;BC;DR0")                               # necessary to prevent problems with subsequent polling in other methods which are called later DR0 must be set!
		self.__parameteranalyzer.write("US DV1;SR1,0")
		self.__parameteranalyzer.write("US DV2;SR2,0")
		self.__parameteranalyzer.write("US DV3;SR3,0")
		self.__parameteranalyzer.write("US DV4;SR4,0")
		self.__parameteranalyzer.write("US DV1,0,"+str(Vgs)+","+str(gatecomp)+",;ST1,1")           # set up gate voltage on SMU1
		self.__parameteranalyzer.write("RG 1,"+str(Vgsrange))
		self.__parameteranalyzer.write("US DV2,0,"+str(Vds)+","+str(draincomp)+",;ST2,1")          # set up drain voltage on SMU2
		self.__parameteranalyzer.write("RG 2,"+str(Vdsrange))
		...
		Id_bias=1.                                                  # current Id (initialized to 1 to ensure at least one measurement loop
		starttime = time.time()
		timemeas=starttime
		while 2.*abs(Id_bias-Id_bias_last)/(abs(Id_bias)+abs(Id_bias_last)) > maxchangeId and (timemeas-starttime)<maxtime:
			Id_bias_last=Id_bias
	...
	#    print "Ialpha is ",Ialpha                                   # debug
			...
			print("from line 121 parameter_analyzer.py Id_bias, Id_bias_last",Id_bias,Id_bias_last)
...
# This is set up to backgate devices
	...
	# set up SMUs for drain and gate
		...
		self.Vds_bias = Vds														# eventually want to read Vgs from the instrument
		self.__parameteranalyzer.write("EM 1,0")								# set to 4200 mode
		#self.__parameteranalyzer.write("IT1;BC;DR0")
		self.__parameteranalyzer.write("IT"+inttime+";BC;DR0")                  # necessary to prevent problems with subsequent polling in other methods which are called later DR0 must be set!
		self.__parameteranalyzer.write("US DV1;SR1,0")
		self.__parameteranalyzer.write("US DV2;SR2,0")
		self.__parameteranalyzer.write("US DV3;SR3,0")
		self.__parameteranalyzer.write("US DV4;SR4,0")
		self.__parameteranalyzer.write("US DV3,0,"+str(Vgs)+","+str(gatecomp)+",;ST3,1")           # set up gate voltage on SMU3
		self.__parameteranalyzer.write("RG 3,"+str(Vgsrange))
		self.__parameteranalyzer.write("US DV2,0,"+str(Vds)+","+str(draincomp)+",;ST2,1")          # set up drain voltage on SMU2
		self.__parameteranalyzer.write("RG 2,"+str(Vdsrange))
...
# set constant bias and read drain and gate currents. This is useful for initial evaluation to determine if FETs should be measured further
	...
	# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("EM 1,0")								# set to 4200 mode
		self.__parameteranalyzer.write("IT1;BC;DR0")                            # necessary to prevent problems with subsequent polling in other methods which are called later DR0 must be set!
		self.__parameteranalyzer.write("IT1")
		self.__parameteranalyzer.write("US DV1;SR1,0")
		self.__parameteranalyzer.write("US DV2;SR2,0")
		self.__parameteranalyzer.write("US DV3;SR3,0")
		self.__parameteranalyzer.write("US DV4;SR4,0")
		self.__parameteranalyzer.write("US DV3,0,"+str(Vgs)+","+str(gatecomp)+",;ST3,1")           # set up gate voltage on SMU3
		self.__parameteranalyzer.write("RG 3,"+str(Vgsrange))
		self.__parameteranalyzer.write("US DV1,0,"+str(Vds0)+","+str(draincomp0)+",;ST1,1")           # set up drain0 voltage on SMU1
		self.__parameteranalyzer.write("RG 1,"+str(Vds0range))
		self.__parameteranalyzer.write("US DV2,0,"+str(Vds1)+","+str(draincomp1)+",;ST2,1")          # set up drain1 voltage on SMU2
		self.__parameteranalyzer.write("RG 2,"+str(Vds1range))
		time.sleep(settlingtime)                                              # power supply settling time before measurement to allow charging of system etc...
...
# measure IV family of curves on topgated FET type structures
	def measure_ivfoc_topgate(self, inttime='2', Vds_start=None, Vds_stop=None, draincomp=None, Vds_npts=None, Vgs_start=None, Vgs_stop=None, gatecomp=None, Vgs_npts=None):
		...
		self.__parameteranalyzer.write("EM 1,1")
# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("DE;CH1,'VG','IG',1,2")                                                             # gate drive channel definition VAR2
		self.__parameteranalyzer.write("SS;VP "+str(Vgs_start)+","+str(Vgs_step)+","+str(Vgs_npts)+","+str(gatecomp))         # gate drive
		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,1")                                                              # drain drive channel definition VAR1
		self.__parameteranalyzer.write("SS;VR1,"+str(Vds_start)+","+str(Vds_stop)+","+str(Vds_step)+","+str(draincomp))         # drain drive
		...
		self.__parameteranalyzer.write("MD;ME1")                                        # trigger for IV measurement
		#self.__parameteranalyzer.write("DR1")
...
# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,2")                                                             # gate drive channel definition VAR2 i.e. SMU3 for gate
		self.__parameteranalyzer.write("SS;VP "+str(Vgs_start)+","+str(Vgs_step)+","+str(Vgs_npts)+","+str(gatecomp))         # gate drive
		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,1")                                                              # drain drive channel definition VAR1 SMU2
		self.__parameteranalyzer.write("SS;VR1,"+str(Vds_start)+","+str(Vds_stop)+","+str(Vds_step)+","+str(draincomp))         # drain drive
...
# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("DE;CH1,'VG','IG',1,1")                          # gate drive channel definition set Vgs set to sweep VAR1
		self.__parameteranalyzer.write("SS;VR1,"+str(Vgs_start)+","+str(Vgs_stop)+","+str(Vgs_step)+","+str(gatecomp))         # gate drive
...
# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,1")                			# gate drive channel definition set to VAR1 sweep
		self.__parameteranalyzer.write("SS;VR1,"+str(Vgs_start)+","+str(Vgs_stop)+","+str(Vgs_step)+","+str(gatecomp))         # gate drive
		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,3")                         # drain drive channel definition VAR1
		self.__parameteranalyzer.write("SS;VC2, "+str(Vds)+","+str(draincomp))         # constant drain voltage drive
...
# Backgate devices use CH3, SMU3 as the gate while topgated devices use CH1, SMU1 as the gate. In all cases, CH2, SMU2 is used as the drain
	def measure_hysteresistimedomain(self, Vds=None, backgated=True, Vgsquiescent=0., timestep=0.01, timequiescent=1.,timeend=1.,Vgs_start=None, Vgs_stop=None, Vgs_step=None, draincomp=None, gatecomp=None):
		...
		if backgated: self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,1")                          # gate drive channel definition set Vgs = constant gate drive on SMU3 to chuck
		else: self.__parameteranalyzer.write("DE;CH1,'VG','IG',1,1")                          # gate drive channel definition set Vgs = constant gate drive on SMU1 to left probe
		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,3")                         # drain drive channel definition VAR1 on SMU2
		self.__parameteranalyzer.write("SS;VC2, "+str(self.Vdsset)+","+str(draincomp))         # constant drain voltage drive
		self.__parameteranalyzer.write("SS;DT 0")                              # set sweep delay=0
		self.__parameteranalyzer.write("SS;HT ",formatnum(holdtime,precision=4,nonexponential=True))            # set holdtime to set quiescent time
		self.__parameteranalyzer.write("SM;WT 0")
		self.__parameteranalyzer.write("SM;IN 0")

		self.__parameteranalyzer.write("RI 2" + "," + formatnum(draincomp,precision=4,nonexponential=True) + "," + formatnum(draincomp,precision=4,nonexponential=True))  # manual set of right device drain current range to turn off autoranging
		if backgated: self.__parameteranalyzer.write("RI 3" + "," + formatnum(gatecomp,precision=4,nonexponential=True) + "," + formatnum(gatecomp,precision=4,nonexponential=True))  # manual set of gate current range to turn off autoranging
		else: self.__parameteranalyzer.write("RI 1" + "," + formatnum(gatecomp,precision=4,nonexponential=True) + "," + formatnum(gatecomp,precision=4,nonexponential=True))  # manual set of gate current range to turn off autoranging
		self.__parameteranalyzer.write("SR 1,1")
		self.__parameteranalyzer.write("SR 2,1")
		self.__parameteranalyzer.write("SR 3,1")
		self.__parameteranalyzer.write("SR 4,1")
		...
		for Vgs in self.Vgssweeparray:
			...
			# configure for sweep
			if backgated:
				self.__parameteranalyzer.write("SS;VL3,1," + formatnum(gatecomp,precision=4,nonexponential=True) + "," + Vgsconstantarray)  # gate drive voltage step
				time.sleep(10*self.ntimepts/4090.)
				self.__parameteranalyzer.write("RI 3" + "," + formatnum(gatecomp, precision=4, nonexponential=True) + "," + formatnum(gatecomp, precision=4, nonexponential=True))  # manual set of gate current range to turn off autoranging
				self.__parameteranalyzer.write("RG 3,0.01")  # appears to get rid of autoscaling
			else:
				self.__parameteranalyzer.write("SS;VL1,1," + formatnum(gatecomp,precision=4,nonexponential=True) + "," + Vgsconstantarray)  # gate drive voltage step
				time.sleep(10*self.ntimepts/4090.)
				self.__parameteranalyzer.write("RI 1" + "," + formatnum(gatecomp, precision=4, nonexponential=True) + "," + formatnum(gatecomp, precision=4, nonexponential=True))  # manual set of gate current range to turn off autoranging
			...
			self.__parameteranalyzer.write("MD;ME1")                                        # trigger for Id vs time measurement
			...
			reread = True
			while reread == True:
				#self.__parameteranalyzer.write("DO 'VD'")
...
# Backgate devices use CH3, SMU3 as the gate while topgated devices use CH1, SMU1 as the gate. In all cases, CH2, SMU2 is used as the drain
	def measure_ivtransferloop_controlledslew(self, backgated=True, Vgsslewrate=None,Vds=None, draincomp=None, Vgs_start=None, Vgs_stop=None, Vgs_step=None, gatecomp=None):
		...
		if backgated:
			...
			self.__parameteranalyzer.write("SS;VL3,1," + formatnum(gatecomp, precision=4, nonexponential=True) + "," + Vgssweeparray)  # gate drive voltage step
			time.sleep(20.*nVgs/4090.)
			self.__parameteranalyzer.write("RI 3" + "," + formatnum(gatecomp, precision=4, nonexponential=True) + "," + formatnum(gatecomp, precision=4, nonexponential=True))  # manual set of gate current range to turn off autoranging
			self.__parameteranalyzer.write("RG 3,0.1")  # manual set of gate current range to turn off autoranging
		else:
			self.__parameteranalyzer.write("DE;CH1,'VG','IG',1,1")  # gate drive channel definition set Vgs = constant gate drive on SMU1 to left probe
			self.__parameteranalyzer.write("SS;VL1,1," + formatnum(gatecomp, precision=4, nonexponential=True) + "," + Vgssweeparray)  # gate drive voltage step
			time.sleep(20.*nVgs/4090.)
			self.__parameteranalyzer.write("RI 1" + "," + formatnum(gatecomp, precision=4, nonexponential=True) + "," + formatnum(gatecomp, precision=4, nonexponential=True))  # manual set of gate current range to turn off autoranging
			self.__parameteranalyzer.write("RG 1,0.1")  # manual set of gate current range to turn off autoranging
		self.__parameteranalyzer.write("RI 2" + "," + formatnum(draincomp, precision=4, nonexponential=True) + "," + formatnum(draincomp, precision=4, nonexponential=True))  # manual set of right device drain current range to turn off autoranging
		self.__parameteranalyzer.write("RG 2,0.1")                  # appears to get rid of autoscaling

		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,3")  # drain drive channel definition VAR1 on SMU2
		self.__parameteranalyzer.write("SS;VC2, " + formatnum(self.Vdsset, precision=4, nonexponential=True) + "," + formatnum(draincomp, precision=4, nonexponential=True))  # constant drain voltage drive
...
# only for devices which require the gate voltage to be appled on the chuck
	def measure_ivtransferloop_backgate(self, inttime=None, Iautorange=True, delayfactor=2, filterfactor=0, integrationtime=1, sweepdelay=0.0,holdtime=0.0, Vds=None, draincomp=None, Vgs_start=None, Vgs_stop=None, Vgs_step=None, gatecomp=None):
		...
		self.__parameteranalyzer.write("EM 1,0")  # set to 4200 mode
		self.__parameteranalyzer.write("BC;DR1")
		self.__parameteranalyzer.write("IT" + inttime)
		self.__parameteranalyzer.write("SM;DM1")
		...
		# configure for dual (loop) sweep
		self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,1")  # gate drive channel definition set Vgs = constant gate drive on SMU3 to chuck
		self.__parameteranalyzer.write("SS;VL3,1," + str(gatecomp) + "," + Vgssweeparray)  # gate drive for sweep
		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,3")  # drain drive channel definition VAR1 on SMU2
		self.__parameteranalyzer.write("SS;VC2, " + str(Vds) + "," + str(draincomp))  # constant drain voltage drive
		self.__parameteranalyzer.write("SS;DT ", str(sweepdelay))
		self.__parameteranalyzer.write("SS;HT ", str(holdtime))
...
# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,2")                                                             # gate drive channel definition VAR2
		self.__parameteranalyzer.write("SS;VP "+str(Vgs_start)+","+str(Vgs_step)+","+str(Vgs_npts)+","+str(gatecomp))         # gate drive
		#self.__parameteranalyzer.write("DT "+str(self.delaytimemeas))           # add delay time to aid settling and reduce propability of compliance due to charging transients

		self.__parameteranalyzer.write("DE;CH2,'VD1','ID1',1,1")                                                              # drain2 drive channel definition VAR1' locked to drain1 VAR1 sweep
		self.__parameteranalyzer.write("SS;VR1,"+str(Vds_start)+","+str(Vds_stop)+","+str(Vds_step)+","+str(draincomp))         # drain2 drive
		#self.__parameteranalyzer.write("DT "+str(self.delaytimemeas))           # add delay time to aid settling and reduce propability of compliance due to charging transients

		self.__parameteranalyzer.write("DE;CH1,'VD0','ID0',1,4")                                                              # drain0 drive channel definition VAR1
		self.__parameteranalyzer.write("SS;RT 1.0")         																	# drain0 drive
		#self.__parameteranalyzer.write("DT "+str(self.delaytimemeas))           # add delay time to aid settling and reduce propability of compliance due to charging transients
		...
		if custom==True and Iautorange==False:                               # then NOT autoranging so set drain and gate compliance and range
			...
			self.__parameteranalyzer.write("RI 2" + "," + str(draincomp) + "," + str(draincomp))  # allow manual set of right device drain current range to turn off autoranging
			self.__parameteranalyzer.write("RI 3" + "," + str(gatecomp) + "," + str(gatecomp))  # allow manual set of right device drain current range to turn off autoranging
		self.__parameteranalyzer.write("SM;XN 'VD0',1,-3.0,0.")                          # configure Keithley 4200 display X axis
		...
		# print("from line 1489 status byte is",sb)
		self.__parameteranalyzer.write("MD;ME1")                                        # trigger for IV measurement
...
# set up SMUs for drain and gate
		...
		self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,1")                			# gate drive channel definition
		self.__parameteranalyzer.write("SS;VR1, "+"".join([str(Vgs_start), ",", str(Vgs_stop), ",", str(Vgs_step), ",", str(gatecomp)]))         # gate drive

		self.__parameteranalyzer.write("DE;CH1,'VD0','ID0',1,3")                         # drain drive channel definition VAR1
		self.__parameteranalyzer.write("SS;VC1, "+"".join([str(Vds), ",", str(draincomp)]))         # constant drain voltage drive

		self.__parameteranalyzer.write("DE;CH2,'VD1','ID1',1,3")                         # drain drive channel definition VAR1
		self.__parameteranalyzer.write("SS;VC2, "+"".join([str(Vds), ",", str(draincomp)]))         # constant drain voltage drive
		...
		if custom==True and Iautorange==False:  # then NOT autoranging so set drain and gate compliance and range
			...
			self.__parameteranalyzer.write("RI 3" + "," + str(gatecomp) + "," + str(gatecomp))  # allow manual set of gate current range to turn off autoranging
		self.__parameteranalyzer.write("SM;LI 'VD0','VG','ID0','IG'")
		self.__parameteranalyzer.write("SM;LI 'VD0','VG','ID1','IG'")
		self.__parameteranalyzer.write("SM;XN 'VG',1,-3.0,0.")                          # configure Keithley 4200 display X axis
		self.__parameteranalyzer.write("SM;YA 'ID0',1,-10u,0.")                          # configure Keithley 4200 display Y axis
		self.__parameteranalyzer.write("SM;YB 'ID1',1,-10u,0.")                          # configure Keithley 4200 display Y axis
		starttime=time.time()                                                      # measure sweep time
		self.__parameteranalyzer.write("MD;ME1")                                        # trigger for transfer curve measurement
		#self.__parameteranalyzer.wait_for_srq(None)                                         # wait for transfer curve data sweep to complete
		self.__panpoll()
...
# Backgate devices use CH3, SMU3 as the gate  CH1 SMU1 and CH2, SMU2 are used as the drains
	def measure_ivtransferloop_dual_controlledslew_backgated(self, Vgsslewrate=None, Vds=None, draincomp=None, Vgs_start=None, Vgs_stop=None, Vgs_step=None, gatecomp=None):
		...
		self.__parameteranalyzer.write("SR 3,0")
		self.__parameteranalyzer.write("SR 4,0")
		self.__parameteranalyzer.write("IT4,0.,0.," + formatnum(PLC, precision=4, nonexponential=True))  # set filterfactor and delay factor both = 0.
		...
		#left drain
		self.__parameteranalyzer.write("RI 1" + "," + formatnum(draincomp, precision=4, nonexponential=True) + "," + formatnum(draincomp, precision=4, nonexponential=True))  # manual set of right device drain current range to turn off autoranging
		self.__parameteranalyzer.write("RG 1,0.01")  # appears to get rid of autoscaling
		self.__parameteranalyzer.write("DE;CH1,'VD0','ID0',1,3")  # drain drive channel definition VAR1 on SMU2
		self.__parameteranalyzer.write("SS;VC1, " + formatnum(self.Vdsset, precision=4, nonexponential=True) + "," + formatnum(draincomp, precision=4, nonexponential=True))  # constant drain voltage drive
...
# swept drain voltage is applied to SMU2
	def measure_burnsweepVds_backgate(self, inttime='2', currentrange=None,delayfactor='2',filterfactor='1',integrationtime=1, Vgs=None, draincomp=None, Vds_start=None, Vds_max=None, Vds_step=None, gatecomp=None):
		...
		print ("IV tranfercurve start") # debug
		self.__parameteranalyzer.write("EM 1,0")								# set to 4200 mode
		self.__parameteranalyzer.write("IT"+inttime+";BC;DR1")                  # setup delays and data acquision timing and averaging for noise reduction
		self.__parameteranalyzer.write("SM;DM1")

# set up SMUs for drain and gate
		self.__parameteranalyzer.write("DE")                                # set to channel definition page
		#self.__parameteranalyzer.write("DT "+str(self.delaytimemeas))           # add delay time to aid settling and reduce propability of compliance due to charging transients
		self.__parameteranalyzer.write("CH1;CH2;CH3;CH4;VS1;VS2;VM1;VM2")     # first undefine all channels

		self.__parameteranalyzer.write("DE;CH3,'VG','IG',1,3")                			# gate drive channel definition set Vgs = constant	SMU3
		self.__parameteranalyzer.write("SS;VC3,"+","+str(Vgs)+","+str(gatecomp))         # gate drive
		self.__parameteranalyzer.write("DE;CH2,'VD','ID',1,1")                         # drain drive channel definition VAR1
...
# the dither voltage is an oscillating series of gate voltages whose amplitude shrinks linearly with time over the dither time so as to reduce or eliminate hysteresis effects at the steady-state gate voltage
	def ditherVgsvoltage(self,gatevoltagechannel=None,gatecomp=None,Vgs=None,Vgspos=None,Vgsneg=None,dithertime=None):
		...
		self.__parameteranalyzer.write("SS;DT 0")  # set sweep delay=0
		self.__parameteranalyzer.write("SS;HT 0")  # set holdtime to set quiescent time
		self.__parameteranalyzer.write("SM;WT 0")
		self.__parameteranalyzer.write("SM;IN 0")
		...
		self.__parameteranalyzer.write("IT4,0,0," + formatnum(PLC, precision=2, nonexponential=True))  # set filterfactor and delay factor both = 0.
		self.__parameteranalyzer.write("SS;VL"+str(gatevoltagechannel)+",1"+formatnum(gatecomp,precision=4,nonexponential=True) + "," + Vgsdither)
		time.sleep(10*npts/4090.)
		self.__parameteranalyzer.write("RI "+str(gatevoltagechannel)+","+formatnum(gatecomp, precision=4, nonexponential=True) + "," + formatnum(gatecomp, precision=4, nonexponential=True))  # manual set of gate current range to turn off autoranging
		self.__parameteranalyzer.write("RG "+str(gatevoltagechannel)+",0.01")  # appears to get rid of autoscaling
		self.__parameteranalyzer.write("MD;ME1")  # trigger for Id vs time measurement
		self.__panpoll()  # wait for transfer curve data sweep to complete
		self.__parameteranalyzer.write("BC")            # throw away data
#######################################################################################################################################################
...
# WARNING! BC,DR1 must be executed as a setup OR this polling WILL NOT WORK
	def __panpoll(self):
		...
		#self.__parameteranalyzer.wait_for_srq()
		#self.__parameteranalyzer.read_stb()
		#print "status byte before loop ",self.__parameteranalyzer.read_stb()
		#while (16 & self.__parameteranalyzer.read_stb() > 0) and ((1 & self.__parameteranalyzer.read_stb()) == 0):
		#while (1 & self.__parameteranalyzer.read_stb()) == 0:
		time.sleep(0.5)
		while True:
