...
pathname = "C:/Users/test/python/data/Wf170meas5"
...
while pr.get_probingstatus()=='probing wafer':
	...
	for Vdsburn in Vdsburnarray:
		...
		iv.writefile_ivtransferloop(pathname,pr.devicename(),pr.wafername(),pr.x(),pr.y(),devicenamemodifier="Vdsburn"+formatnum(Vdsburn,precision=2))
		...
		#iv.writefile_burnonefinalvaluessweepVds(pathname=pathname,wafername=cascade.wafername(),devicename=cascade.devicename(),closefile=True) # just close the file (nothing else)
