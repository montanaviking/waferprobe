...
class PowerCompressionTunedFocus(FocusTuner,HP438A):
	def __init__(self, rm=None, tunertype='load', calfactorA=97.6, calfactorB=97.55, loglin='LG', cascadefiles_port1=None, cascadefiles_port2=None, spectrum_analyzer_input_attenuation=None, spectrum_analyzer_cal_factor=None, powerlevellinear_min=None, powerlevellinear_max=None, maxpower=None, comp_fillin_step=None, powerlevel_step=None, source_calibration_factor=None):
		...
		self._spectrum_analyzer_cal_factor=spectrum_analyzer_cal_factor
		self._spectrum_analyzer_input_attenuation=spectrum_analyzer_input_attenuation
		if spectrum_analyzer_cal_factor!=None and spectrum_analyzer_input_attenuation!=None:          # then use the spectrum analyzer
			...
			HP438A.__init__(self,rm=rm,calfactorA=calfactorA,calfactorB=calfactorB,loglin=loglin)           # set up power meter to use instead of spectrum analyzer
...
#   Vgs, Vds, draincomp, gatecomp, are respectively the gate voltage, drain voltage, drain current compliance limit (A), and gate current compliance limit (A)
	def measurePcomp(self,ps=None, fractlinfitlower=0.,fractlinfitupper=1.,compressiontarget=1.,maxcompressionlevelerror=0.1, powermeterfilterfactor=None,output_reflection=None,frequency=None,Vgs=0.,Vds=0.,draincomp=0.1,gatecomp=0.1,plotcompression=True,maxiterations=30):
		...
		prfmin=self.readpower(sensor='A')
