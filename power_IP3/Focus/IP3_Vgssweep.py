...
class IP3_Vgssweep(object):
	def __init__(self,rm=None, lower_rf_synthesizer=None, upper_rf_synthesizer=None, powermeter=None, vgsgen=None,bias=None,sa=None,scope=None,tuner=None,                              # handles for instruments used in this test. When no instrument handle is passed, this class creates the handle in the constructor
				 ...
				 center_frequency=None,frequency_spread=None,spectrum_analyzer_cal_factor=None, source_cal_factor=None,usecas1=True,usecas2=True,
				 Vgsperiod=1E-3,Vgsmin=None,Vgsmax=None,syscheckonly=False,holdtime=0.):
		self.holdtime=holdtime              # time to apply bias and wait prior to taking measurments
		if probefile!=None:
			cas1=[[probefile,'flip']]
		else: cas1=None
		...
		# cascade 2-ports used on input and output of tuner
		S1=None
		S2=None
		self.__DUToutputcircuit=None
		if cas1!=None and len(cas1)>0 and usecas1:      # cas1 is the 2-port between the DUT output and tuner input. Usually includes the wafer probe. If no tuner cas1 = None and is NOT used
		# add the first of the cascaded 2-ports to form S1 - the two-port attached to port 1 of the tuner which is facing the DUT
		# cas1 and S1 are the Used ONLY if a tuner is present, i.e. DUT output is tuned. Otherwise if no tuner is used, this is a 50ohm system and S1=None and cas1=None
		...
		# if this is a 50ohm test system with NO tuner (DUT output at 50ohms during test) then cas2 and S2 contain the 2-port which ties the output of the DUT to the input of the spectrum analyzer
		...
		if tuner==True:
			...
			self.__tuner=False
			S1=None                             # we are NOT using S1 if the tuner is not present and S2 contains the 2-port between the DUT output and tuner input
			self.__DUToutputcircuit=S2          # When the tuner is not used (50ohm test), S2 is the 2-port between the DUT output and spectrum analyzer input
			if self.__DUToutputcircuit==None:   raise ValueError("ERROR no 2-port S-parameters provided between DUT and spectrum analyzer in 50ohm system (no tuner)")
		...
		self.spectrum_analyser_input_attenuation=spectrum_analyser_input_attenuation
...
#
# When the DUT is measured without a tuner, i.e. 50ohms at the DUT output as indicated by self.__tuner=False, then substitute self.__DUToutputcircuit for self.__tuner for the effective tuner S-parameters
#
	def measureTOI_gain(self, Pin=None, output_reflection=None,Vds=0., draincomp=0.1,noavg_dist=64,setup=True,quickmeasurement=True):
		if self.__tuner!=False and (output_reflection==None or len(output_reflection)<1): raise ValueError("ERROR! no tuner reflections given")
		if Pin==None:
		...
		if self.__tuner!=False:
			...
			self.actual_Z=gammatoYZ(self.actualrcoefMA,Z=True)
			self.actual_Y=gammatoYZ(self.actualrcoefMA,Z=False)
			...
			self.pos="0,0"                                  # dummy value for tuner position when tuner is not used
			self.actualrcoefMA=complex(0,0)                 # this is a 50ohm system so the reflection coefficient = 0
			self.actualrcoefRI=complex(0,0)                 # this is a 50ohm system so the reflection coefficient = 0
			self.actual_Z=complex(50,0)
			self.actual_Y=complex(20E-3,0)
			tuner_loss=-20.*np.log10(abs(self.__DUToutputcircuit[str(int(1E-6*self.centfreq))][1,0]))             # with no tuner, i.e. 50ohm system, The DUT to spectrum analyzer loss is S21 of the 2-port between DUT and spectrum analyzer. This is in the negative of dB of gain so loss gives a positive number here. Also note that the self.__DUToutputcircuit[str(center frequency in MHz)][x,y] where x and y are indices of the S-parameters-1
		####
		...
		# set up Vgs (gate voltage triangular wavefunction) sweep generator
			...
			self.Vgstimestamps_raw,self.Id_raw,self.Vgs_raw = ramped_sweep_gate(scope=self.__scope, pulsegenerator=self.__vgsgen, period=self.Vgsperiod,Vgslow=self._Vgsmin,Vgshigh=self._Vgsmax,average=8,volttocurrentcalibrationfactor=volttocurrentcalibrationfactor, scopetriggerHFfilteron=True)
			...
			self.Vgstimestamps_raw,self.Id_raw,self.Vgs_raw = ramped_sweep_gate(scope=self.__scope, pulsegenerator=self.__vgsgen, period=self.Vgsperiod,Vgslow=self._Vgsmin,Vgshigh=self._Vgsmax,average=8,setupautoscale=False,volttocurrentcalibrationfactor=volttocurrentcalibrationfactor, scopetriggerHFfilteron=True)
			if not self.syscheckonly: self.Idaverage,Voutactual,self.drainstatus=self.__bias.get_Iout(channel=1)
				 # turn on drain bias and allow drain current to stabilize
		...
		time.sleep(self.holdtime)               # hold bias and gate drive to allow for settling
		...
		satimestamps_raw,pfundlower_read,self._flower,self.actualresolutionbandwidth,self.actualvideobandwidth=self.__sa.measure_amplitude_waveform(frequency=self._flower,sweeptime=self.Vgsperiod,resolutionbandwidth=resolutionbandwidthsetting,
								videobandwidth=videobandwidthsetting,attenuation=self.spectrum_analyser_input_attenuation,numberofaverages=16)
		if not quickmeasurement:
			...
			t,pfundupper_read,self._fupper,dummy1,dummy2=self.__sa.measure_amplitude_waveform(frequency=self._fupper,sweeptime=self.Vgsperiod,resolutionbandwidth=resolutionbandwidthsetting,
								 videobandwidth=videobandwidthsetting,attenuation=self.spectrum_analyser_input_attenuation,referencelevelguess=referencelevelsetting,numberofaverages=16)
		...
		t,p3rdlower_read,flower_3rd,dummy1,dummy2=self.__sa.measure_amplitude_waveform(frequency=flower_3rd,sweeptime=self.Vgsperiod,resolutionbandwidth=resolutionbandwidthsetting,
								 videobandwidth=videobandwidthsetting,attenuation=self.spectrum_analyser_input_attenuation,referencelevelguess=referencelevelsetting,numberofaverages=noavg_dist)
...
# frequency in Hz
#
	def measure_gainonly(self, Pin=None, output_reflection=None, Vgsmin=None,Vgsmax=None,Vds=0., draincomp=0.1, setup=True,frequency=None):
		...
		self.Idaverage=0.
		self.drainstatus='N'
		if Vgsmin!=None: self._Vgsmin=Vgsmin
		if Vgsmax!=None: self._Vgsmax=Vgsmax
		if self._Vgsmin==None: raise ValueError("ERROR! no value given for Vgsmin")
		if self._Vgsmax==None: raise ValueError("ERROR! no value given for Vgsmax")
		# set up tuner reflection
		...
		if self.__tuner!=False:
			...
			self.actual_Z=complex(50,0)
			self.actual_Y=complex(20E-3,0)
			tuner_loss=-10.*np.log10(self.__DUToutputcircuit[str(int(1E-6*self.centfreq))][1,0])             # with no tuner, i.e. 50ohm system, The DUT to spectrum analyzer loss is S21 of the 2-port between DUT and spectrum analyzer. This is in the negative of dB of gain so loss gives a positive number here. Also note that the self.__DUToutputcircuit[str(center frequency in MHz)][x,y] where x and y are indices of the S-parameters-1
		####
		outcorrectiontotal=tuner_loss+self.__spectrum_analyser_cal_factor      # power output at DUT output - power reading on spectrum analyzer
		resolutionbandwidthsetting=50./self.Vgsperiod
		resolutionbandwidthsetting_autoscale=4./self.Vgsperiod
		...
		satimestamps_raw,pfund_read,frequency,self.actualresolutionbandwidth,self.actualvideobandwidth=self.__sa.measure_amplitude_waveform(frequency=frequency,sweeptime=self.Vgsperiod,resolutionbandwidth=resolutionbandwidthsetting,
								videobandwidth=videobandwidthsetting,attenuation=self.spectrum_analyser_input_attenuation,numberofaverages=16)      # frequency in Hz
		maxtimestampint=min(self.Vgstimestamps_raw[-1],satimestamps_raw[-1])             # find the maximum interpolation time stamp
