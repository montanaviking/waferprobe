...
import time
	...
	# source_calibration_factor_uppperfreq=PDUTin-PsynBset where PDUTin is the power at the DUT input, as read by the power meter and corrected for probe loss when synthesizer B power (upper frequency) is set to PsynAset and synthesizer B (upper frequency) is off. self._sourcecalfactor is expected to be set to a negative dB quantity
...
class IP3_Tuned(FocusTuner):
	def __init__(self,rm=None,tunertype='load',cascadefiles_port1=None,cascadefiles_port2=None, spectrum_analyser_input_attenuation=10., number_of_averages=8, powerlevel_start=None, powerlevel_stop=None, powerlevel_step=None,
	             center_frequency=None, frequency_spread=None,spectrum_analyzer_cal_factor=None, inputcallowerfreq=None,inputcalupperfreq=None, inputsetestsystem_TOI=None):
		...
		self._sourcecalfactor_lowerfreq=source_calibration_factor_lowerfreq    # dB difference between the power at the DUT's input (port 1) - as measured on the power meter, and the power setting on the RF synthesizer A. Almost always a negative dB number
		self._sourcecalfactor_upperfreq=source_calibration_factor_upperfreq    # dB difference between the power at the DUT's input (port 1) - as measured on the power meter, and the power setting on the RF synthesizer A. Almost always a negative dB number
		self._deltacal_LU=source_calibration_factor_lowerfreq-source_calibration_factor_upperfreq
	...
	def TOIsearch(self,initial_output_reflections=None,fractlinfitlower=0.,fractlinearfitupper=0.,expected_gain=0.,plotgraphs=False,stopping_criteria=0.2,maximum_tuner_reflection=0.92,maxnumberoftries=12):
	...
	def measureTOI(self,output_reflection=None,fractlinfitlower=0.,fractlinearfitupper=0.,expected_gain=0.,plotgraphs=False,cleardata=True):
		...
		# loop through specified output reflection coefficients
		for ref in output_reflection:
			...
			print("from line 206 in IP3_focus_tuned.py pos, gamma=",self.actualrcoefMA[pos],pos)
			...
			plevelstart = self._plevelstart-self._sourcecalfactor_lowerfreq  # plevelstart is the lowest power direct front-panel setting used on the lower-frequency synthesizer it is the requested lowest power corrected by the losses from synthesizer to the DUT input.
			plevelmax= self._plevelstop-self._sourcecalfactor_lowerfreq				# plevelmax is the highest power direct front-panel setting used on the lower-frequency synthesizer. it is the requested highest power corrected by the losses from synthesizer to the DUT input.
			...
			self.__rfhigh.set_power(plevelmax+self._deltacal_LU)
			while self.__rflow.set_power(plevelmax)=='unleveled' or self.__rfhigh.set_power(plevelmax+self._deltacal_LU)=='unleveled':
				# power level setting is beyond at least one of the synthesizer's capability so reduce the maximum requested power
				print ("Warning: maximum requested lower-frequency synthesizer power beyond capability so reducing maximum power set at synthesizers to", plevelmax)
			...
			while plevel>=plevelstart:                          # start third order intercept test at highest power plevel is the raw power setting of the synthesizer which will give the desired power at the DUT input and is the direct power setting of the lower tone.
				...
				#print "set res 1"
				reflevel=10.*int((plevel+expected_gain+self._reflevelgaincorrection+self._sourcecalfactor_lowerfreq-self._sacalfactor-self._tuner_loss)/10.)     # set spectrum analyzer reference level to nearest increment of 10dB
				...
				#print "set res 2"
				pwrhigh = plevel+self._deltacal_LU   # attempt to correct for errors on two tone signal levels to attempt to make both of the two tone have the same levels self._delpowm is the difference in power settings of power of low- power of high frequency RF synthesizer (two-tone fundamentals)
				...
				self.pinDUT[pos].append(plevel+self._sourcecalfactor_lowerfreq)		# power into DUT with synthesizer to DUT losses calibrated out (should be actual power to DUT input in dBm)
				...
				print("tuner reflection IP3_focus_tuned.py line 337 sys",self.sys[pos][-1],self._sysTOI,self.TOIptt[pos][-1])
