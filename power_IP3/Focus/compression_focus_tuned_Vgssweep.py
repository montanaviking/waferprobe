...
synthesizerpowersetforcalibration=-40.    # used to calibrate synthesizer set values to corresponding RF power levels at DUT input. This is the dBm power setting of the RF synthesizers.
...
class Pcomp_Vgssweep(object):
	def __init__(self,rm=None, rf_synthesizer=None, powermeter=None, vgsgen=None,bias=None,sa=None,scope=None,tuner=None,                              # handles for instruments used in this test. When no instrument handle is passed, this class creates the handle in the constructor
		...
		if tuner!=None and tuner!=False:
			...
			try: self.__tuner=FocusTuner(S1=S1,S2=S2,tunertype='load')
			except: raise ValueError("ERROR! Requested tuner but no tuner available")
		elif tuner==False:
			self.__tuner=False
...
# Pin is the RF power (dBm) for the RF input signal
# output reflection is magnitude and angle (degrees) when the tuner is used
#
	def measure_gainonly(self, Pin=None, output_reflection=None, Vds=0., draincomp=0.1, setup=True):
		...
		# set up Vgs (gate voltage triangular wavefunction) sweep generator
			...
			print("Holding for "+formatnum(number=self.holdtime,type="int")+" sec")
			time.sleep(self.holdtime)               # hold bias and gate drive to allow for settling for the first time only
		else:
