...
#
def get_noisemeter_noiseparameter_coldsource(tunercalfile=None,cascadefiles_port1=None,cascadefiles_port2=None,reflectioncoefficients=None, tunerIP=("192.168.1.31",23), referencelevel=-50.,
                                             inputfile_noisemeterdiodedata=None,resolutionbandwidthsetting=4E6,videobandwidthsetting=10,average=2,maxdeltaNF=0.1):
	...
	gammanm={str(nmgb['frequenciesMHz'][i]):nmgb['gammanm'][i] for i in range(0,len(nmgb['frequenciesMHz'])) for nmgb['frequenciesMHz'][i] in frequencies}   # gammanm['frequencyMHz'] is the reflection coefficient at the input of the composite noise meter real+jimaginary
	...
	for p in select_pos:
		...
		maxfreq,maxsig,frequencies_sa,noise=spectrumanalyzer.measure_spectrum(numberofaverages=average,centfreq=centfreq,span=span,videobandwidth=videobandwidthsetting,resolutionbandwidth=resolutionbandwidthsetting,referencelevel=referencelevel,attenuation=0,preamp=True)
		...
		for f in frequencies_str:               # loop through selected frequencies str representing the frequency in MHz
			...
			Fnm[f][pos]=noise_interp[f]*np.square(abs(1-gammatunerRI[f][pos]*gammanm[f]))/(GB[f]*(1-np.square(abs(gammatunerRI[f][pos]))))          # noise figure of composite noise meter, linear not dB
			FnmdB[f][pos]=lintodB(Fnm[f][pos])          # measured noise figure of composite noise meter in dB
	...
	for f in frequencies_str:
		...
		NPnm[f]['gopt']=npx[1]
		NPnm[f]['Rn']=npx[2]
		#NPnm[f]=solvefornoiseparameters(Fgamma=Fgamma,type='weighted',maxdeltaNF=maxdeltaNF)
	...
	return {'NPnm':NPnm,'NFcalcfromNP':NFcalcfromNPnm,'NF':FnmdB,'gammatunerRI':gammatunerRI,'gammatunerMA':gammatunerMA,'noise':noise_interp}
