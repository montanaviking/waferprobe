...
from calculate_noise_parameters_focus import calculate_nffromnp, calculate_gavail,calculate_NP_DUT,calculate_NP_noisemeter
...
class NoiseParameters(FocusTuner,NoiseMeter,Pna):
	def __init__(self,rm=None,navgPNA=16,navgNF=16,usePNA=False,ENR=14.79,cascadefiles_port1=None,cascadefiles_port2=None,system_noiseparametersfile=None,reflectioncoefficients=None,tunercalfile=None):
		...
		super(NoiseParameters,self).__init__(S1=casaded2port1,S2=casaded2port2,tunertype='source',tunercalfile=tunercalfile)           # set up the tuner with the tuner calibration file tunerfile and the noise meter with the ENR
...
# DUTSpar is a set of previously-measured DUT S-parameters
	def measure_noiseparameters(self,frequenciesMHz=None,DUTSpar=None):
		...
		if frequenciesMHz!=None and len(frequenciesMHz)>0:
			frequencies=[str(int(f)) for f in frequenciesMHz]    # convert to str list
		if DUTSpar!=None and len(DUTSpar)>0:
			frequencies=[str(int(f)) for f in list(set(DUTSpar.keys())&set(list(self.tuner_data.keys()))&set(frequenciesMHz))]
			...
			if frequencies==None: frequencies=list(set(fSpara)&set(list(self.tuner_data.keys())))
			else: frequencies=list(set(fSpara)&set(list(self.tuner_data.keys()))&set(frequencies))

		frequencies=list(map(str,sorted(list(map(int,[int(f) for f in frequencies])))))                       # sort frequencies in ascending value. Needed to use map() to sort frequencies since they are of type str
		...
		self.resettuner()                                                 # set tuner to the reset position
